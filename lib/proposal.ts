...
export function generateProposal(input: string | ProposalInput) {
  ...
  const hours = getHoursAdvice(difficulty, data.deliveryTime);
  ...
  const questions = [
    ...
    "What exact result should be considered done for this task?",
  ...
  const risks = [
    tooShort
      ? "Job Posting 信息太短。建议先让客户补充技术栈、现状截图、访问权限和验收标准。"
      : "",
    ...matchedRisks.map(
      (keyword) => `出现高风险关键词：${keyword}。不要站外付款、站外沟通，也不要接受不合理免费测试。`,
    ),
    ...complexSignals.map((keyword) => `出现复杂项目信号：${keyword}。新手不建议直接承诺完整交付。`),
  ].filter(Boolean);
...
I prefer to keep the process transparent and avoid over-promising.`;
  ...
  return {
    feasible,
    difficulty,
    beginnerAdvice: beginnerRecommended
      ? "可以谨慎投标，但要先问清楚范围，并把承诺控制在自己能验证的小交付内。"
      : "不建议新手直接投标。建议先补充信息、拆小范围，或换一个更清楚的小项目。",
    hours,
    price,
    risks: risks.length ? risks : ["暂未发现明显高风险词，但仍需要人工审核客户需求和平台规则。"],
    questions,
    proposal: baseProposal,
    shortProposal:
      "Hi, I can review the issue, confirm the scope, and help with a careful fix after checking the current setup.",
...
To keep this safe, I would start with a short inspection and only proceed after the scope is clear.`,
    notRecommendedReason: beginnerRecommended
      ? ""
      : "需求不清晰、风险偏高或复杂度超出新手阶段，直接投标容易造成交付压力。先询问范围或选择更小、更明确的任务会更稳妥。",
  };
}

function getHoursAdvice(difficulty: string, deliveryTime: DeliveryTime) {
  if (difficulty === "复杂") return "16 小时以上，建议先拆分评估";
  if (deliveryTime === "24h") return difficulty === "简单" ? "2-5 小时" : "6-12 小时";
  if (difficulty === "简单") return "2-6 小时";
  return "6-16 小时";
}

function getPriceAdvice(strategy: PricingStrategy, difficulty: string) {
  if (difficulty === "复杂") {
    return "不要直接报固定低价，建议先做 paid discovery 或请客户拆小范围。";
  }

  if (strategy === "starter") {
    return "新手可以低价起步，但不要低到覆盖不了沟通、测试和平台成本。";
  }

  if (strategy === "premium") {
    return "高价值报价需要清晰案例、交付边界和较强沟通能力。";
  }
