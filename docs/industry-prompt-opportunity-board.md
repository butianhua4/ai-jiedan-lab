# Industry Prompt Opportunity Board

Generated at: 2026-06-07T17:54:20.874Z

This report is read-only. It turns broad business prompt demand into specific page ideas and prompt-pack review targets.

## Guardrails

- Auto create articles: false
- Auto edit articles: false
- Auto mark review: false
- Auto publish: false
- Traffic claim: No measured traffic, impressions, rankings, clicks, or revenue are claimed.
- Note: Read-only opportunity board. It turns broad prompt demand into specific page and prompt-pack ideas without editing or publishing articles.

## Source Evidence

- Search date: 2026-06-07
- Search note: External sources were used as current category signals only; they are not traffic-volume evidence.

Official prompt sources:

- https://platform.openai.com/docs/guides/prompt-engineering
- https://platform.openai.com/docs/guides/prompt-generation
- https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/overview
- https://support.google.com/docs/answer/15013615
- https://adoption.microsoft.com/en-us/copilot/prompt-gallery/

Market signal sources:

- https://pmtly.com/: Prompt libraries commonly split business prompts by marketing, sales, finance, HR, customer support, operations, legal, project management, development, and data analytics.
- https://www.fwdslash.ai/prompt-template: Recent prompt-template pages emphasize reusable templates with role context, variables, output format, and model-agnostic usage.
- https://sensara.io/prompts/: Business prompt libraries are organized around client communication, marketing, operations, hiring, reports, customer service, sales, and content creation.
- https://adoption.microsoft.com/en-us/copilot/prompt-gallery/: Microsoft Copilot's prompt gallery exposes role and task filters such as communication, finance, HR, IT, operations, content creation, reporting, and summarization.
- https://www.promptfluent.com/browse: Enterprise prompt-library examples stress role-specific prompt governance for sales, HR, finance, operations, legal, customer service, and software development.

## Summary

- departmentLanes: 12
- itemsWithHumanBoundary: 12
- itemsWithInputOutputStructure: 12
- itemsWithReviewPackCandidate: 11
- itemsWithSourceTargets: 12
- opportunities: 12
- promptModules: 60
- promptPublicArticles: 0
- searchQueryFamilies: 60
- unsafeItems: 0
- zeroPublicCoverageItems: 12

## Unsafe Items

- none

## Top Opportunities

| Score | Public | Review candidates | Query families | Lane | Primary query | Deliverable |
| --- | --- | --- | --- | --- | --- | --- |
| 226 | 0 | 3 | 5 | customer-service-ai-prompts | 客服 AI 回复模板 | Customer support prompt pack with ticket triage, empathy reply, escalation, refund boundary, and knowledge-base update. |
| 216 | 0 | 2 | 5 | sales-ai-prompts | 销售 AI 提示词 | Sales prompt pack with prospect research, discovery questions, objection handling, follow-up, and CRM summary. |
| 212 | 0 | 2 | 5 | hr-ai-prompts | HR AI 提示词 | HR prompt pack with JD draft, interview rubric, onboarding plan, performance review, and policy summary. |
| 208 | 0 | 2 | 5 | operations-ai-prompts | 运营 AI 提示词 | Operations prompt pack with SOP, weekly report, meeting summary, project risk list, and retrospective. |
| 206 | 0 | 2 | 5 | finance-ai-prompts | 财务 AI 提示词 | Finance prompt pack with variance narrative, budget review, cost driver summary, forecast assumptions, and board memo. |
| 204 | 0 | 2 | 5 | education-ai-prompts | 教师 AI 提示词 | Education prompt pack with lesson plan, quiz, student feedback, learning plan, and teaching-material rewrite. |
| 202 | 0 | 2 | 5 | product-manager-ai-prompts | 产品经理 AI 提示词 | Product prompt pack with PRD, user stories, competitor notes, acceptance criteria, and launch checklist. |
| 198 | 0 | 1 | 5 | marketing-ai-prompts | 营销 AI 提示词 | Marketing prompt pack with campaign brief, SEO outline, ad copy, content calendar, and post-campaign review. |

## All Opportunities

| Score | Public | Review candidates | Query families | Lane | Primary query | Deliverable |
| --- | --- | --- | --- | --- | --- | --- |
| 226 | 0 | 3 | 5 | customer-service-ai-prompts | 客服 AI 回复模板 | Customer support prompt pack with ticket triage, empathy reply, escalation, refund boundary, and knowledge-base update. |
| 216 | 0 | 2 | 5 | sales-ai-prompts | 销售 AI 提示词 | Sales prompt pack with prospect research, discovery questions, objection handling, follow-up, and CRM summary. |
| 212 | 0 | 2 | 5 | hr-ai-prompts | HR AI 提示词 | HR prompt pack with JD draft, interview rubric, onboarding plan, performance review, and policy summary. |
| 208 | 0 | 2 | 5 | operations-ai-prompts | 运营 AI 提示词 | Operations prompt pack with SOP, weekly report, meeting summary, project risk list, and retrospective. |
| 206 | 0 | 2 | 5 | finance-ai-prompts | 财务 AI 提示词 | Finance prompt pack with variance narrative, budget review, cost driver summary, forecast assumptions, and board memo. |
| 204 | 0 | 2 | 5 | education-ai-prompts | 教师 AI 提示词 | Education prompt pack with lesson plan, quiz, student feedback, learning plan, and teaching-material rewrite. |
| 202 | 0 | 2 | 5 | product-manager-ai-prompts | 产品经理 AI 提示词 | Product prompt pack with PRD, user stories, competitor notes, acceptance criteria, and launch checklist. |
| 198 | 0 | 1 | 5 | marketing-ai-prompts | 营销 AI 提示词 | Marketing prompt pack with campaign brief, SEO outline, ad copy, content calendar, and post-campaign review. |
| 188 | 0 | 1 | 5 | software-development-ai-prompts | 编程 AI 提示词 | Software prompt pack with bug reproduction, code review, test cases, refactor plan, and release note. |
| 186 | 0 | 1 | 5 | ecommerce-ai-prompts | 电商 AI 提示词 | E-commerce prompt pack with product title, detail page, review mining, FAQ, and after-sales response. |
| 184 | 0 | 1 | 5 | legal-contract-ai-prompts | 合同 AI 提示词 | Legal operations prompt pack with clause summary, risk questions, negotiation prep, and contract handoff memo. |
| 170 | 0 | 0 | 5 | data-analysis-ai-prompts | 数据分析 AI 提示词 | Data analysis prompt pack with metric explanation, SQL thinking, anomaly review, dashboard narrative, and executive summary. |

## Opportunity Packets

### 客服 AI 回复模板

- Lane: customer-service-ai-prompts
- Audience: support teams, after-sales teams, customer success teams
- Page type: department prompt library
- Priority score: 226
- Public matches: 0
- Recommended action: Use the matched draft candidates as the first review targets; expand the page into a role-specific prompt pack during human review.
- Human boundary: Create or review only as draft/noindex/humanReviewRequired. Stop before mark:review and stop before publish until explicit human approval.

Search queries:

- 客服 AI 回复模板
- 客服 AI 提示词
- customer support AI prompts
- 售后回复 AI prompt
- support ticket classification prompt

Prompt modules:

- complaint response
- support ticket classifier
- refund escalation checker
- customer success plan
- FAQ updater

User input fields:

- customer message
- policy excerpt
- order context
- support channel
- severity
- allowed next actions

Output blocks:

- ticket category
- draft reply
- escalation reason
- policy check
- knowledge-base note

Risk controls:

- Require user-provided facts and mark unknowns instead of inventing business data.
- Include output format, review criteria, and escalation boundary in every prompt.
- Keep human approval for customer, employee, financial, legal, medical, or operational decisions.
- Do not claim guaranteed traffic, ranking, revenue, hiring, legal, medical, or conversion outcomes.
- Refund, privacy, abuse, and safety issues must route to a human owner.

Matched review candidates:

- 全行业 AI 提示词模板库怎么做：销售、运营、客服、HR、财务和教育都能用 (content/blog/industry-ai-prompts-template-library-2026.mdx)
- 客服 AI 提示词模板：回复草稿、工单分类、情绪安抚和升级判断 (content/blog/customer-service-ai-prompts-guide.mdx)
- 电商 AI 提示词模板：商品标题、详情页、评价分析和售后回复 (content/blog/ecommerce-ai-prompts-guide.mdx)

### 销售 AI 提示词

- Lane: sales-ai-prompts
- Audience: sales teams, founders, account managers
- Page type: department prompt library
- Priority score: 216
- Public matches: 0
- Recommended action: Use the matched draft candidates as the first review targets; expand the page into a role-specific prompt pack during human review.
- Human boundary: Create or review only as draft/noindex/humanReviewRequired. Stop before mark:review and stop before publish until explicit human approval.

Search queries:

- 销售 AI 提示词
- ChatGPT prompts for sales
- 销售话术 AI prompt
- 客户跟进 AI 提示词
- sales objection handling prompt

Prompt modules:

- prospect research brief
- discovery call planner
- objection handler
- follow-up sequence
- win/loss recap

User input fields:

- target customer
- offer
- deal stage
- known objections
- tone
- next step

Output blocks:

- prospect brief
- discovery questions
- objection responses
- follow-up email
- CRM update

Risk controls:

- Require user-provided facts and mark unknowns instead of inventing business data.
- Include output format, review criteria, and escalation boundary in every prompt.
- Keep human approval for customer, employee, financial, legal, medical, or operational decisions.
- Do not claim guaranteed traffic, ranking, revenue, hiring, legal, medical, or conversion outcomes.
- Do not present persuasive scripts as guaranteed conversion tactics.

Matched review candidates:

- 全行业 AI 提示词模板库怎么做：销售、运营、客服、HR、财务和教育都能用 (content/blog/industry-ai-prompts-template-library-2026.mdx)
- 销售 AI 提示词模板：客户画像、跟进话术、异议处理和会议纪要 (content/blog/sales-ai-prompts-guide.mdx)

### HR AI 提示词

- Lane: hr-ai-prompts
- Audience: HR teams, recruiters, people operations
- Page type: department prompt library
- Priority score: 212
- Public matches: 0
- Recommended action: Use the matched draft candidates as the first review targets; expand the page into a role-specific prompt pack during human review.
- Human boundary: Create or review only as draft/noindex/humanReviewRequired. Stop before mark:review and stop before publish until explicit human approval.

Search queries:

- HR AI 提示词
- 招聘 AI prompt
- 面试题 AI 提示词
- HR Copilot prompts
- employee review prompt

Prompt modules:

- JD writer
- interview rubric builder
- onboarding checklist
- performance review assistant
- policy explainer

User input fields:

- role
- seniority
- must-have skills
- company context
- evaluation rubric
- legal constraints

Output blocks:

- job description
- screening criteria
- interview questions
- onboarding checklist
- review notes

Risk controls:

- Require user-provided facts and mark unknowns instead of inventing business data.
- Include output format, review criteria, and escalation boundary in every prompt.
- Keep human approval for customer, employee, financial, legal, medical, or operational decisions.
- Do not claim guaranteed traffic, ranking, revenue, hiring, legal, medical, or conversion outcomes.
- Avoid discriminatory criteria and require HR/legal review for people decisions.

Matched review candidates:

- 全行业 AI 提示词模板库怎么做：销售、运营、客服、HR、财务和教育都能用 (content/blog/industry-ai-prompts-template-library-2026.mdx)
- 人力招聘 AI 提示词模板：JD、简历初筛、面试题和培训材料 (content/blog/hr-recruiting-ai-prompts-guide.mdx)

### 运营 AI 提示词

- Lane: operations-ai-prompts
- Audience: operations managers, project owners, internal workflow teams
- Page type: department prompt library
- Priority score: 208
- Public matches: 0
- Recommended action: Use the matched draft candidates as the first review targets; expand the page into a role-specific prompt pack during human review.
- Human boundary: Create or review only as draft/noindex/humanReviewRequired. Stop before mark:review and stop before publish until explicit human approval.

Search queries:

- 运营 AI 提示词
- SOP AI prompt
- 周报 AI 提示词
- meeting summary prompt
- operations AI prompts

Prompt modules:

- SOP builder
- weekly report drafter
- meeting summary to action items
- risk register
- process retrospective

User input fields:

- process goal
- current notes
- owners
- timeline
- constraints
- acceptance criteria

Output blocks:

- SOP steps
- weekly summary
- owners and dates
- risk list
- retrospective actions

Risk controls:

- Require user-provided facts and mark unknowns instead of inventing business data.
- Include output format, review criteria, and escalation boundary in every prompt.
- Keep human approval for customer, employee, financial, legal, medical, or operational decisions.
- Do not claim guaranteed traffic, ranking, revenue, hiring, legal, medical, or conversion outcomes.
- Operational actions must keep owners, deadlines, and acceptance criteria explicit.

Matched review candidates:

- 全行业 AI 提示词模板库怎么做：销售、运营、客服、HR、财务和教育都能用 (content/blog/industry-ai-prompts-template-library-2026.mdx)
- 制造业 AI 提示词模板：SOP、质检记录、设备故障和生产复盘 (content/blog/manufacturing-ai-prompts-guide.mdx)

### 财务 AI 提示词

- Lane: finance-ai-prompts
- Audience: finance teams, founders, business analysts
- Page type: department prompt library
- Priority score: 206
- Public matches: 0
- Recommended action: Use the matched draft candidates as the first review targets; expand the page into a role-specific prompt pack during human review.
- Human boundary: Create or review only as draft/noindex/humanReviewRequired. Stop before mark:review and stop before publish until explicit human approval.

Search queries:

- 财务 AI 提示词
- finance AI prompts
- 报表摘要 AI prompt
- 预算复盘 AI 提示词
- financial analysis prompt

Prompt modules:

- variance analysis explainer
- budget review memo
- cost anomaly triage
- forecast assumption checker
- board finance summary

User input fields:

- source numbers
- period
- comparison baseline
- known anomalies
- audience
- decision needed

Output blocks:

- variance summary
- cost drivers
- assumptions
- risk flags
- board-ready narrative

Risk controls:

- Require user-provided facts and mark unknowns instead of inventing business data.
- Include output format, review criteria, and escalation boundary in every prompt.
- Keep human approval for customer, employee, financial, legal, medical, or operational decisions.
- Do not claim guaranteed traffic, ranking, revenue, hiring, legal, medical, or conversion outcomes.
- AI output must not replace accounting, tax, audit, or investment judgment.

Matched review candidates:

- 全行业 AI 提示词模板库怎么做：销售、运营、客服、HR、财务和教育都能用 (content/blog/industry-ai-prompts-template-library-2026.mdx)
- 财务 AI 提示词模板：报表摘要、费用分类、预算复盘和风险清单 (content/blog/finance-ai-prompts-guide.mdx)

### 教师 AI 提示词

- Lane: education-ai-prompts
- Audience: teachers, trainers, course creators
- Page type: department prompt library
- Priority score: 204
- Public matches: 0
- Recommended action: Use the matched draft candidates as the first review targets; expand the page into a role-specific prompt pack during human review.
- Human boundary: Create or review only as draft/noindex/humanReviewRequired. Stop before mark:review and stop before publish until explicit human approval.

Search queries:

- 教师 AI 提示词
- 教育 AI 提示词
- 备课 AI prompt
- lesson plan AI prompt
- quiz generator prompt

Prompt modules:

- lesson planner
- quiz generator
- rubric builder
- student feedback assistant
- learning plan adjuster

User input fields:

- grade level
- topic
- learning objective
- student context
- timebox
- assessment method

Output blocks:

- lesson objective
- activity plan
- quiz items
- feedback notes
- adaptation plan

Risk controls:

- Require user-provided facts and mark unknowns instead of inventing business data.
- Include output format, review criteria, and escalation boundary in every prompt.
- Keep human approval for customer, employee, financial, legal, medical, or operational decisions.
- Do not claim guaranteed traffic, ranking, revenue, hiring, legal, medical, or conversion outcomes.
- Teachers must verify difficulty, answers, accessibility, and student suitability.

Matched review candidates:

- 全行业 AI 提示词模板库怎么做：销售、运营、客服、HR、财务和教育都能用 (content/blog/industry-ai-prompts-template-library-2026.mdx)
- 教育 AI 提示词模板：备课、教案、测验、反馈和学习计划 (content/blog/education-ai-prompts-guide.mdx)

### 产品经理 AI 提示词

- Lane: product-manager-ai-prompts
- Audience: product managers, founders, business analysts
- Page type: department prompt library
- Priority score: 202
- Public matches: 0
- Recommended action: Use the matched draft candidates as the first review targets; expand the page into a role-specific prompt pack during human review.
- Human boundary: Create or review only as draft/noindex/humanReviewRequired. Stop before mark:review and stop before publish until explicit human approval.

Search queries:

- 产品经理 AI 提示词
- PRD AI prompt
- 需求分析 AI 提示词
- user story prompt
- 产品文档 AI prompt

Prompt modules:

- PRD drafter
- user story generator
- acceptance criteria builder
- competitor analysis
- launch checklist

User input fields:

- user segment
- problem
- business goal
- constraints
- evidence
- success metric

Output blocks:

- problem statement
- user stories
- acceptance criteria
- tradeoffs
- launch checklist

Risk controls:

- Require user-provided facts and mark unknowns instead of inventing business data.
- Include output format, review criteria, and escalation boundary in every prompt.
- Keep human approval for customer, employee, financial, legal, medical, or operational decisions.
- Do not claim guaranteed traffic, ranking, revenue, hiring, legal, medical, or conversion outcomes.
- Product requirements must stay traceable to real user evidence and measurable acceptance criteria.

Matched review candidates:

- 软件开发 AI 提示词模板：需求拆解、代码审查、Bug 排查和测试用例 (content/blog/software-development-ai-prompts-guide.mdx)
- 产品经理 AI 提示词模板：需求分析、PRD、竞品、用户故事和验收标准 (content/blog/product-manager-ai-prompts-guide.mdx)

### 营销 AI 提示词

- Lane: marketing-ai-prompts
- Audience: marketing teams, content teams, SEO operators
- Page type: department prompt library
- Priority score: 198
- Public matches: 0
- Recommended action: Use the matched draft candidates as the first review targets; expand the page into a role-specific prompt pack during human review.
- Human boundary: Create or review only as draft/noindex/humanReviewRequired. Stop before mark:review and stop before publish until explicit human approval.

Search queries:

- 营销 AI 提示词
- SEO AI 提示词
- 广告文案 AI prompt
- marketing AI prompts
- 内容运营 AI 提示词

Prompt modules:

- campaign planner
- SEO outline builder
- ad copy generator
- social content calendar
- campaign retrospective

User input fields:

- audience
- offer
- brand voice
- proof points
- channels
- constraints

Output blocks:

- campaign brief
- SEO outline
- ad variants
- content calendar
- review checklist

Risk controls:

- Require user-provided facts and mark unknowns instead of inventing business data.
- Include output format, review criteria, and escalation boundary in every prompt.
- Keep human approval for customer, employee, financial, legal, medical, or operational decisions.
- Do not claim guaranteed traffic, ranking, revenue, hiring, legal, medical, or conversion outcomes.
- Claims, metrics, and customer proof must be backed by provided evidence.

Matched review candidates:

- 营销 AI 提示词模板：选题、广告文案、SEO 和活动复盘怎么写 (content/blog/marketing-ai-prompts-guide.mdx)

### 编程 AI 提示词

- Lane: software-development-ai-prompts
- Audience: developers, QA engineers, engineering leads
- Page type: department prompt library
- Priority score: 188
- Public matches: 0
- Recommended action: Use the matched draft candidates as the first review targets; expand the page into a role-specific prompt pack during human review.
- Human boundary: Create or review only as draft/noindex/humanReviewRequired. Stop before mark:review and stop before publish until explicit human approval.

Search queries:

- 编程 AI 提示词
- 代码审查 AI prompt
- Bug 排查 AI 提示词
- 测试用例 AI prompt
- developer AI prompts

Prompt modules:

- bug triage
- code review checklist
- test-case generator
- refactor plan
- release-note writer

User input fields:

- repo context
- error
- expected behavior
- constraints
- test framework
- security assumptions

Output blocks:

- bug hypothesis
- test cases
- review findings
- risk notes
- release summary

Risk controls:

- Require user-provided facts and mark unknowns instead of inventing business data.
- Include output format, review criteria, and escalation boundary in every prompt.
- Keep human approval for customer, employee, financial, legal, medical, or operational decisions.
- Do not claim guaranteed traffic, ranking, revenue, hiring, legal, medical, or conversion outcomes.
- Code suggestions require tests, security review, and repository-specific validation.

Matched review candidates:

- 软件开发 AI 提示词模板：需求拆解、代码审查、Bug 排查和测试用例 (content/blog/software-development-ai-prompts-guide.mdx)

### 电商 AI 提示词

- Lane: ecommerce-ai-prompts
- Audience: e-commerce operators, marketplace sellers, product-content teams
- Page type: department prompt library
- Priority score: 186
- Public matches: 0
- Recommended action: Use the matched draft candidates as the first review targets; expand the page into a role-specific prompt pack during human review.
- Human boundary: Create or review only as draft/noindex/humanReviewRequired. Stop before mark:review and stop before publish until explicit human approval.

Search queries:

- 电商 AI 提示词
- 商品标题 AI prompt
- 详情页 AI 文案
- 评价分析 AI 提示词
- ecommerce AI prompts

Prompt modules:

- product-title optimizer
- detail-page copywriter
- review miner
- FAQ builder
- after-sales reply drafter

User input fields:

- product facts
- target buyer
- platform rules
- reviews
- brand tone
- forbidden claims

Output blocks:

- product title
- detail copy
- review themes
- FAQ entries
- after-sales reply

Risk controls:

- Require user-provided facts and mark unknowns instead of inventing business data.
- Include output format, review criteria, and escalation boundary in every prompt.
- Keep human approval for customer, employee, financial, legal, medical, or operational decisions.
- Do not claim guaranteed traffic, ranking, revenue, hiring, legal, medical, or conversion outcomes.
- Product facts, claims, policies, and platform rules must be manually verified.

Matched review candidates:

- 电商 AI 提示词模板：商品标题、详情页、评价分析和售后回复 (content/blog/ecommerce-ai-prompts-guide.mdx)

### 合同 AI 提示词

- Lane: legal-contract-ai-prompts
- Audience: legal operations, contract managers, business owners
- Page type: department prompt library
- Priority score: 184
- Public matches: 0
- Recommended action: Use the matched draft candidates as the first review targets; expand the page into a role-specific prompt pack during human review.
- Human boundary: Create or review only as draft/noindex/humanReviewRequired. Stop before mark:review and stop before publish until explicit human approval.

Search queries:

- 合同 AI 提示词
- 法务 AI prompt
- 合同风险 AI 提示词
- 条款摘要 AI prompt
- legal AI prompts

Prompt modules:

- clause summarizer
- contract risk question list
- redline prep
- negotiation brief
- lawyer handoff memo

User input fields:

- contract excerpt
- jurisdiction context
- business objective
- risk tolerance
- counterparty
- questions for counsel

Output blocks:

- clause summary
- risk questions
- missing info
- negotiation notes
- lawyer handoff

Risk controls:

- Require user-provided facts and mark unknowns instead of inventing business data.
- Include output format, review criteria, and escalation boundary in every prompt.
- Keep human approval for customer, employee, financial, legal, medical, or operational decisions.
- Do not claim guaranteed traffic, ranking, revenue, hiring, legal, medical, or conversion outcomes.
- Contract output must not be framed as legal advice and must route to qualified review.

Matched review candidates:

- 法务合同 AI 提示词模板：条款摘要、风险清单和修改问题 (content/blog/legal-contract-ai-prompts-guide.mdx)

### 数据分析 AI 提示词

- Lane: data-analysis-ai-prompts
- Audience: data analysts, operations analysts, business teams
- Page type: department prompt library
- Priority score: 170
- Public matches: 0
- Recommended action: Create a new draft candidate for this department lane, then run the normal draft guardrails and human review workflow.
- Human boundary: Create or review only as draft/noindex/humanReviewRequired. Stop before mark:review and stop before publish until explicit human approval.

Search queries:

- 数据分析 AI 提示词
- SQL AI prompt
- 指标分析 AI 提示词
- 数据报告 AI prompt
- data analytics AI prompts

Prompt modules:

- metric explainer
- SQL plan assistant
- anomaly triage
- dashboard narrative
- executive data summary

User input fields:

- metric
- dataset description
- time range
- known caveats
- business question
- desired chart or summary

Output blocks:

- metric definition
- analysis plan
- SQL outline
- anomaly hypotheses
- executive summary

Risk controls:

- Require user-provided facts and mark unknowns instead of inventing business data.
- Include output format, review criteria, and escalation boundary in every prompt.
- Keep human approval for customer, employee, financial, legal, medical, or operational decisions.
- Do not claim guaranteed traffic, ranking, revenue, hiring, legal, medical, or conversion outcomes.
- SQL and conclusions must be executed and verified by a human against the real data source.

Matched review candidates:

- none

