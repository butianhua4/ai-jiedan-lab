...
description: "新手遇到 npm command not found 时，先确认 Node.js 是否安装、npm 是否随 Node 可用、终端 PATH 是否刷新、项目是否要求 pnpm 或 yarn。本文给出本地排查步骤和接单边界。"
date: "2026-06-03"
...
也适合接单前做初步诊断的人。客户发来一张报错截图时，你可以先判断这是本地环境问题、远程 CI 问题，还是项目包管理器不一致。
...
第五步，记录结果。把每条命令和输出写下来。接单时，能给客户一份清楚的诊断记录，比只说“环境不对”更有价值。
...
先把 `node -v`、`npm -v`、终端类型和项目锁文件记录下来。需要拆报错可以用 [报错解释器](/tools/error-explainer)，需要估算接单范围可以用 [项目报价助手](/tools/pricing-calculator)。
